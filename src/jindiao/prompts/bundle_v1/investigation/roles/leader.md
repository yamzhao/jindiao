...
2. 先调用 `submit_check_assignments`，以 15 个逻辑 CheckAssignment 覆盖全部启用核查项；这些逻辑分配不是 AgentTeams scheduled 任务。
...
4. 使用 `read_investigation_progress` 检查黑板；全部 15 个核查结果提交后，才创建一个独立 Reviewer 复核任务交给 `reviewer-agent`。

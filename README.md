...
基于 openJiuwen agent-Core AgentTeams 与 DeepSearch 的单机高代码企业信用与风控尽调应用。系统先按产品依赖共享采集并冻结企业事实，再让一个真实调查 Agent 或一个真实调查团队在同一证据快照上完成固定核查，最后生成与产品原型一致的申报方案、风险卡片和 Markdown 报告。

> 这是小盾未来产品工程演示项目，不是生产征信服务，输出不能替代人工授信、审计或法律意见。仓库内企业均为虚构 Mock 数据。
...
`amount` 为万元，后端统一换算，报告金额仍为元。`customerName` 必填且不能为空，仅按客户名称确认公司主体；`uscc` 可提交但不参与主体匹配或幂等哈希，报告信用代码使用查询结果。不再接受 `enterprise`、`business_context`、`region` 等旧入参。必要控制参数仍在一级，完整列表见 API 文档第 2.1 节。下方 v1 示例保留原嵌套契约。

现有表单的业务品种、金额、期限、客户经理和支行直接进入报告。后端结合已审核风险生成六类建议（额度、利率、授信期限、贷款期限、担保及还款方式），按 1 万元以内、1～3 个月的候选规则校验；利率使用待审批的文字建议，模型缺项或失败时提供明确标记的规则补全。其他未提供的事实保持为空，前端可显示 `-`。建议来源、拒绝授信时的不适用项及字段映射见 [API 文档](docs/api/README.md)。

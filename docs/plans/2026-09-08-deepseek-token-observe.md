...
**Goal:** Switch the existing local OpenAI-compatible route to deepseek-v4-flash and, as explicitly requested, disable local numeric token-budget enforcement while retaining accounting and other execution guards.
...
- [x] Set `.env` MODEL_NAME=deepseek-v4-flash and JINDIAO_ENFORCE_TOKEN_BUDGET=false only; preserve route and credentials. Add strict default to .env.example and document current threshold formula.
